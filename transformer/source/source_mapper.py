...
class SourceMapper:
    def run(self, config: SourceMapperConfig) -> Dict[str, pd.DataFrame]:
        ...
        if config.nan_check:
            ...
            for df in dataframes:
                try:
                    validator.NaNValidator().validate(
                        segment=df, field_name='ALL', arguments={}, frames=dataframes
                    )
                except ValidationError as e:
                    errors.append(e)
            if len(errors) > 0:
                raise ValidationFailureError(
                    f'Nan Validation failed for {len(errors)} segments.', errors
                )
    ...
    def _format(
        self, config: [SourceFormatterConfig], file_name
    ) -> Dict[str, pd.DataFrame]:
        dataframes = {}
        for cfg in config:
            dataframes[cfg.segment] = getattr(source_formatter, cfg.name)().run(
                cfg, file_name
            )
        return dataframes

    def _convert(
        self, config: [ConverterConfig], dataframes: [str, pd.DataFrame]
    ) -> Dict[str, pd.DataFrame]:
        for cfg in config:
            dataframes[cfg.segment][cfg.field_name] = getattr(
                converter, cfg.name
            )().run(cfg, dataframes[cfg.segment][cfg.field_name])
        return dataframes

    def _validate(
        self, config: [ValidatorConfig], dataframes: [str, pd.DataFrame]
    ) -> None:
        errors = []
        for cfg in config:
        ...
        if len(errors) > 0:
            raise ValidationFailureError(
                f'There are {len(errors)} pre-validation errors. {errors}', errors
            )

    def _trim(self, dataframes: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        for df in dataframes:
            dataframes[df] = dataframes[df].applymap(
